...
// Convert file to base64 so image persists across sessions
...
export default function CreatePost() {
  ...
  const handleFile = async (file) => {
    ...
    setMediaFile(file);
    // Convert to base64 in background for persistent storage
    if (!isVideo) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // Use base64 for images (persists after refresh), blob URL for videos
      const finalMediaUrl = mediaBase64 || mediaPreview;
      createPost({
        userId: user.id,
  ...
  if (step === 3) {
    return (
      <div className="create-page">
        <div className="post-success fade-in">
          <div className="post-success-icon">🎉</div>
          <h2 className="post-success-title">Post Shared!</h2>
          <p className="post-success-sub">Your post is now live on the feed.</p>
          {tags.length > 0 && (
            <p className="post-success-tags">
              {tags.length} product{tags.length > 1 ? 's' : ''} tagged ✅
            </p>
          )}
          <div className="spinner" style={{margin: '20px auto 0'}} />
          <p style={{color:'var(--text3)', fontSize:'0.82rem', marginTop:'10px'}}>Redirecting to feed...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="create-layout">
          ...
          <div className="create-details-col">
            ...
            <div className="create-actions">
              <button className="btn-ghost" onClick={() => { setStep(1); setMediaPreview(''); setMediaBase64(''); setTags([]); }}>
                Change Media
              </button>
              <button
